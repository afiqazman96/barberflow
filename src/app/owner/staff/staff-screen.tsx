...
/**
 * The profile fields an owner can correct after the fact. Kept as strings
 * because they are what the inputs hold; the target is parsed on submit, where
 * a blank box means "no target" rather than zero.
 */
type EditStaffForm = {
  name: string;
  phone: string;
...
/**
 * One directory row as the screen needs it.
 *
 * Who exists, and their account state, comes from the database. The live shop
 * state around them — status, chair, today's takings — is still the client
 * store's, so a member the store knows keeps the numbers the rest of the
 * portal is showing; one it does not (created from another device, say) starts
 * from what the database last recorded.
 */
function toMember(
  entry: StaffDirectoryEntry,
  local: StaffMember | undefined,
  ...
  const live: Pick<
    ...
    | "monthlyCommission"
  > = local ?? {
    branchId: entry.branchId ?? fallbackBranchId,
...
export function OwnerStaffScreen({
  ...
  // The database decides who is on the team; the store fills in the live shop
  // state for the people it already knows about.
  const staff = useMemo(() => {
    const fallbackBranchId = branches[0]?.id ?? "";
    return directory.map((entry) =>
      toMember(
        entry,
        storeStaff.find((m) => m.id === entry.id),
        fallbackBranchId,
      ),
    );
  }, [directory, storeStaff, branches]);

  // Live status is the store's, which only knows the people it has seen this
  // session; anyone else falls back to the status the database recorded.
  const statusOf = (member: StaffMember) =>
    staffStatuses[member.id] ?? member.status;

  /**
   * Runs a server action with the screen's shared failure handling: a rejected
   * write reports why, an unreachable server says so, and either way the
  ...
  /**
   * Seeded from the directory row rather than the merged member: the member's
   * `monthlyTarget` is coalesced to 0 for the progress bar, so "no target set"
   * would come back as a 0 the owner never typed.
   */
  function openEdit(member: StaffMember) {
    const entry = directory.find((e) => e.id === member.id);
    setEditForm({
      name: entry?.name ?? member.name,
      phone: entry?.phone ?? "",
      specialty: entry?.specialty ?? "",
      monthlyTarget:
        entry?.monthlyTarget === null || entry?.monthlyTarget === undefined
          ? ""
          : String(entry.monthlyTarget),
    });
    setEditOpen(true);
  }
  ...
  async function handleEditProfile(e: React.FormEvent) {
    ...
    const target =
      editForm.monthlyTarget.trim() === ""
        ? null
        : Number(editForm.monthlyTarget);

    const result = await runAction(
      () =>
        updateStaffProfile(member.id, {
          name,
        ...
        }),
      {
        ...
        offline: "Details were not saved — check your connection and try again.",
      },
    );
    if (!result?.ok) return;
    ...
    closeEdit();
    toast.success("Details updated", { description: name });
  }
  ...
  /**
   * Each of these writes to the backend first and mirrors into the store only
   * once it succeeds, so a rejected write leaves the screen showing what the
   * database actually holds rather than an optimistic lie.
   */
  async function handleStatusChange(staffId: string, status: StaffStatus) {
    const member = staff.find((s) => s.id === staffId);
    if (status !== "available" && status !== "break") return;

    const result = await runAction(() => setStaffStatus(staffId, status), {
      failed: "Could not update the status",
      offline: "Status was not changed — check your connection and try again.",
    });
    if (!result?.ok) return;

    updateStaffStatus(staffId, status);
    toast.success("Status updated", {
      description: `${member?.name} → ${status}`,
    });
  }

  async function handleChairAssign(staffId: string, chairId: string) {
    const member = staff.find((s) => s.id === staffId);
    const chair = chairs.find((c) => c.id === chairId);

    const result = await runAction(
      () => assignStaffChair(staffId, chairId || null),
      {
        failed: "Could not assign the chair",
        offline: "Chair was not assigned — check your connection and try again.",
      },
    );
    if (!result?.ok) return;
  ...
  async function handleTransferBranch(newBranchId: string) {
    if (!selected || newBranchId === selected.branchId) return;
    const member = selected;

    const result = await runAction(
      () => transferStaffBranch(member.id, newBranchId),
      {
        failed: "Could not transfer this staff member",
        offline:
          "Staff was not transferred — check your connection and try again.",
      },
    );
    if (!result?.ok) return;
    ...
    updateStaff(member.id, { branchId: newBranchId, chairId: null });
    const branchName = branches.find((b) => b.id === newBranchId)?.name;
    toast.success("Staff transferred", {
      description: `${member.name} → ${branchName ?? "branch"}`,
    });
  }
  ...
  async function handleAddStaff(e: React.FormEvent) {
    ...
    const result = await runAction(
      ...
      {
        failed: "Could not create the real login",
        offline:
          "Staff was not created — check your connection and try again.",
      },
    );
    if (!result?.ok) return;
    const realStaffId = result.data.staffId;
    ...
    const member = addStaff({
      ...
      branchId: form.branchId,
      // Same value the server just stored, so the mirror cannot drift from
      // what the directory will report on the next read.
      monthlyTarget: Number(form.monthlyTarget) || 0,
      chairId: form.chairId || null,
      status: "off-duty",
    });
  ...
  async function applyToggleActive() {
    ...
    const result = await runAction(() => setStaffActive(selected.id, next), {
      failed: "Could not update the real login",
      offline:
        "Account status was not changed — check your connection and try again.",
    });
    if (!result?.ok) return;
  ...
  async function handleResetPassword(e: React.FormEvent) {
    ...
    const result = await runAction(
      () => resetStaffPassword(selected.id, resetPassword),
      {
        failed: "Could not reset the real login",
        offline:
          "Password was not reset — check your connection and try again.",
      },
    );
    if (!result?.ok) return;
          ...
          <div className="space-y-5">
            ...
            <div className="rounded-xl border border-[var(--border)] p-4">
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  <Pencil className="h-4 w-4 text-[var(--gold)]" />
                ...
                <Button
                  ...
                  variant="ghost"
                  size="sm"
                  onClick={() => (editOpen ? closeEdit() : openEdit(selected))}
                >
              ...
              {editOpen ? (
                <form onSubmit={handleEditProfile} className="mt-3 space-y-3">
                  <div>
                  ...
                  <div>
                    <Label htmlFor="edit-target">Monthly Target (RM)</Label>
                    <Input
                      id="edit-target"
                      type="number"
                      min={0}
                ...
                <p className="mt-1 text-xs text-[var(--text-muted)]">
                  Name, phone, specialty and monthly target. The login email is
                  fixed; the password is reset below.
                </p>
              )}
            </div>
