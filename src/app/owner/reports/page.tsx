...
export default function OwnerReportsPage() {
  ...
  const rangedQueue = useMemo(
    () => queue.filter((q) => inRange(q.createdAt)),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [queue, from, to],
  );
  const rangedBookings = useMemo(
    () => bookings.filter((b) => inRange(b.date)),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [bookings, from, to],
  );
  ...
  const reports = useMemo(() => {
    const map: Record<Exclude<TabId, "overview">, BuiltReport> = {
      ...
      queue: buildQueueReport(rangedQueue),
      bookings: buildBookingReport(rangedBookings),
    };
    return map;
  }, [
    sales,
            ...
            <div className="space-y-6">
              ...
              <Card className="p-4">
                <p className="text-sm text-[var(--text-muted)]">
                  Pick a tab above for a detailed table —{" "}
                  ...
                  Services, Products, Commission, Queue, and Appointments. Every
                  report honours the date range and exports to Excel or PDF.
                </p>
              </Card>
