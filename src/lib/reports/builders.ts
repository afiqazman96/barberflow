...
export function buildSalesReport(sales: Sale[], branchName?: string): BuiltReport {
  ...
  const serviceCharge = sales.reduce((sum, s) => sum + (s.serviceCharge ?? 0), 0);
  const tax = sales.reduce((sum, s) => sum + (s.tax ?? 0), 0);
  const tips = sales.reduce((sum, s) => sum + (s.tip ?? 0), 0);
  ...
  return {
    ...
    columns: [
      ...
      { key: "serviceCharge", header: "Svc Charge", align: "right" },
      { key: "tax", header: "SST", align: "right" },
      { key: "tip", header: "Tip", align: "right" },
    ...
    rows: sales.map((s) => ({
      ...
      serviceCharge: money(s.serviceCharge ?? 0),
      tax: money(s.tax ?? 0),
      tip: money(s.tip ?? 0),
      total: money(s.total),
      paymentMethod: s.paymentMethod.toUpperCase(),
      card: s.card
        ? [s.card.scheme, s.card.last4 && `····${s.card.last4}`]
            .filter(Boolean)
            .join(" ")
        : "",
      commission: money(s.commission),
      status: s.voided ? `Voided — ${s.voided.reason}` : "Completed",
    })),
    summary: [
      { label: "Transactions", value: String(sales.length) },
      { label: "Gross sales", value: money(total) },
      { label: "Service charge", value: money(serviceCharge) },
      { label: "SST collected", value: money(tax) },
